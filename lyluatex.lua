...
function latex.includepdf(pdfname, range, papersize, caption)
    if caption ~= '' then warn([[Captions are not supported with
fullpage scores.]]) end
    local noautoscale = ''
...
function latex.includesystems(filename, range, protrusion, staffsize,
    indent, halign, environment, env_opts, caption, captionbefore)
...
\par\vspace{%spt plus %spt minus %spt}
    ...
    tex.sprint(latex.wrap_figure(texoutput, environment, env_opts,
        caption, captionbefore))
end

function latex.label(label, labelprefix)
    if label then tex.sprint('\\label{'..
        labelprefix..label..'}%%') end
end

ly.verbenv = {[[\begin{verbatim}]], [[\end{verbatim}]]}
function latex.verbatim(verbatim, ly_code, intertext, addversion)
    if verbatim then
...
function latex.wrap_figure(systems, environment, env_opts,
    caption, captionbefore)
    if environment == '' then return systems end
    local b_caption = ''
    local a_caption = ''
    if captionbefore then
...
function Score:write_latex(do_compile)
    ...
    if self.insert == 'fullpage' then
        latex.includepdf(self.output, self:_range(), self.papersize, self.caption)
    elseif self.insert == 'systems' then
        latex.includesystems(
            self.output, self:_range(), self:_protrusion(),
            self.staffsize, self.indent,
            self.halign, self.environment, self['env-opts'],
            self.caption, self.captionbefore
        )
    else -- inline
